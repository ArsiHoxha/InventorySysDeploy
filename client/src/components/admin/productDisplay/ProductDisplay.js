...
const ProductData = () => {
  ...
  const handleDelete = (productId) => {
    ...
    if (isConfirmed) {
      axios.delete(`https://inventorysysdeploy-1.onrender.com/products/${productId}`)
        .then(response => {
                ...
                {filteredProducts.map((product) => (
                  <tr key={product._id}>
                    <td className="py-2 px-4 border-b border-gray-200">
                      <img className="h-16 w-16 object-cover rounded" src={`https://inventorysysdeploy-1.onrender.com/uploads/${product.productImg}`} alt={product.productNameTxt} />
                    </td>
