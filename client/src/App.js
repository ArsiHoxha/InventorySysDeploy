...
function App() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get('https://rrobotika.onrender.com/auth/google/success', { withCredentials: true });
        setUser(res.data);
        if (res.data) {
          navigate('/profile');
        }
  ...
  const handleLogin = () => {
    const googleAuthURL = 'https://rrobotika.onrender.com/auth/google';
