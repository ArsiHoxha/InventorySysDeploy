...
function App() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get('https://inventorysysdeploy.onrender.com/auth/google/success', { withCredentials: true });
        setUser(res.data);
        if (res.data) {
          navigate('/profile');
        }
  ...
  const handleLogin = () => {
    const googleAuthURL = 'https://inventorysysdeploy.onrender.com/auth/google';
    window.open(googleAuthURL, '_self');
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-700">
      <header className="w-full  bg-gray-800 py-4 text-white text-center ">
        <h1 className="text-3xl font-black">Rrobotika Hf</h1>
      </header>
